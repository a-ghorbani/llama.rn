...
std::map<lm_ggml_backend_buffer_type_t, size_t> llama_model::memory_breakdown() const {
    ...
    for (const lm_ggml_backend_buffer_ptr & buf_ptr : pimpl->bufs) {
    ...
    llm_build_grok(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (model.layers[il].ffn_up) {
                lm_ggml_tensor * ffn_out = build_ffn(cur,
                        model.layers[il].ffn_up,   NULL, NULL,
                        model.layers[il].ffn_gate, NULL, NULL,
                        model.layers[il].ffn_down, NULL, NULL,
                        NULL,
                ...
                cur = lm_ggml_scale(ctx0, lm_ggml_add(ctx0, ffn_out, moe_out), std::sqrt(2) / 2);
                cb(cur, "ffn_out", il);
            } else {
                cur = moe_out;
            }
    ...
    llm_build_qwen(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                lm_ggml_tensor * Qcur = lm_ggml_view_3d(ctx0, cur, n_embd_head, n_head,    n_tokens, n_embd_head*sizeof(float), cur->nb[1], 0*sizeof(float)*(n_embd));
                lm_ggml_tensor * Kcur = lm_ggml_view_3d(ctx0, cur, n_embd_head, n_head_kv, n_tokens, n_embd_head*sizeof(float), cur->nb[1], 1*sizeof(float)*(n_embd));
                lm_ggml_tensor * Vcur = lm_ggml_view_3d(ctx0, cur, n_embd_head, n_head_kv, n_tokens, n_embd_head*sizeof(float), cur->nb[1], 2*sizeof(float)*(n_embd));
    ...
    llm_build_jais(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // self-attention
            {
                ...
                lm_ggml_tensor * Qcur = lm_ggml_view_3d(ctx0, cur, n_embd_head, n_head,    n_tokens, n_embd_head*sizeof(float), cur->nb[1], 0*cur->nb[0]*(n_embd));
                lm_ggml_tensor * Kcur = lm_ggml_view_3d(ctx0, cur, n_embd_head, n_head_kv, n_tokens, n_embd_head*sizeof(float), cur->nb[1], 1*cur->nb[0]*(n_embd));
                lm_ggml_tensor * Vcur = lm_ggml_view_3d(ctx0, cur, n_embd_head, n_head_kv, n_tokens, n_embd_head*sizeof(float), cur->nb[1], 1*cur->nb[0]*(n_embd + n_embd_gqa));
...
struct llm_build_nemotron_h : public llm_graph_context_mamba {
    ...
    lm_ggml_tensor * build_ffn_layer(
              lm_ggml_tensor * cur,
        const llama_model & model,
        const int           il) {
...
private:
    lm_ggml_tensor * build_plamo2_attn_layer(
        ...
        // self-attention
        {
            ...
            lm_ggml_tensor * Vcur = lm_ggml_view_3d(ctx0, qkv, n_embd_head_v, n_head_kv, n_tokens, n_embd_head_v * sizeof(float), qkv->nb[1], v_offset * lm_ggml_element_size(qkv));
...
struct llm_build_grovemoe : public llm_graph_context {
    llm_build_grovemoe(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            lm_ggml_tensor * probs = build_lora_mm(model.layers[il].ffn_gate_inp, cur); // [n_expert, n_tokens]
            cb(probs, "ffn_moe_logits", il);
            ...
            cur = lm_ggml_add(ctx0, cur, lm_ggml_scale(ctx0, moe_out, hparams.expert_group_scale));
